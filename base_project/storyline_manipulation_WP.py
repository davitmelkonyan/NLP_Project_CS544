#This code is used for appyling different type of proposed manipulattions on the plots of the WPstories
import torch
import os
import numpy as np
import nltk
...
class Plt_manipulations():
	...
	def __init__(self, COMET_model_file, COMET_sampling_algorithm, device):
		...
		fr = open("/content/drive/My Drive/Colab Notebooks/NLP_Project/Plot-guided-Coherence-Evaluation-main/Data_/conceptnet_antonym.txt", "r")	
	...
	def repetition(self, plots):
		...
		#In WPStory dataset all the sentences' plots of a story are combined with "#"
		...
		#we repeat each selected plt in 33% of sentences of stories in the WPStories dataset
...
if __name__=="__main__":
	...
	fr_plots = open(os.path.join(args.data_dir, args.fname +'.source'), 'r',encoding="mbcs")
	lines_plots = fr_plots.readlines()

	fr_stories = open(os.path.join(args.data_dir, args.fname+'.target'), 'r',encoding="mbcs")
	...
	fw_plts = open(os.path.join(args.data_dir+'ManPlts/', output_file), 'w',encoding="mbcs")
