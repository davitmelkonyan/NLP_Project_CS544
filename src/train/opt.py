    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                exp_avg = torch.add(input = exp_avg,other = grad, alpha = 1 - beta1)

                #exp_avg_sq.mul_(beta2).addcmul_(grad, grad,1 - beta2)
                exp_avg_sq = torch.mul(exp_avg_sq,beta2)
                exp_avg_sq = torch.addcmul(input = exp_avg_sq,tensor1 = grad,tensor2 = grad,value = 1 - beta2)
                ...
                #p.data = torch.addcmul(input = exp_avg_sq,tensor1 = grad,tensor2 = grad,value = 1 - beta2)
                p.data = torch.addcdiv(input = p.data, tensor1=exp_avg, tensor2=denom,value = -step_size)

                # Add weight decay at the end (fixed version)
                if (len(p.size()) > 1 or group['vector_l2']) and group['l2'] > 0:
                    p.data = torch.add(p.data,-lr_scheduled * group['l2'])
